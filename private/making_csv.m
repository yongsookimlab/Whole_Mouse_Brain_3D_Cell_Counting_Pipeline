function [] = making_csv(cell_locations, csv_name, anno, out_csv_file,targeting_resolution) 
...
% B_loc = accumarray(loca, vess_heat_map(non_zeros), size(ROI_table.id));
% transformed_label_loc = accumarray(loca, 1, size(ROI_table.idx));
cell_locations = ceil(cell_locations);
% size(anno)
% max(cell_locations)
cell_location_ind = sub2ind(size(anno),cell_locations(:,1),cell_locations(:,2),cell_locations(:,3));

[logi, loca] = ismember(anno(cell_location_ind), ROI_table.id);

transformed_cell_loc = accumarray(loca(logi), 1, size(ROI_table.idx));
